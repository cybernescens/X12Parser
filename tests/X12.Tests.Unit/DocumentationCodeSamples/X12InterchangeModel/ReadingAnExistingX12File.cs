...
";
    ...
    public void OutlineIsSameAsOriginal()
    {
      ...
      var interchange = parser.ParseMultiple(new MemoryStream(Encoding.ASCII.GetBytes(inquiry))).First();

      var interchangeFromOutline =
        parser.ParseMultiple(new MemoryStream(Encoding.ASCII.GetBytes(inquiryOutline))).First();
    ...
    public void Read270Test()
    {
      ...
      var transaction = interchange.FunctionGroups.First().Transactions.First();
      var bht = transaction.Segments.First();

      Assert.AreEqual("10001234", bht.GetElement(3));

      var subscriberLoop = transaction.FindHLoop("3");

      var subscriberNameLoop = subscriberLoop.Loops.First();
    ...
    public void Create270Test()
    {
      ...
      */
      ...
      var Message = new Interchange(DateTime.Now, 31, false) {
        ...
        SecurityInfo = string.Format(
          "{0,-10}",
          " "), //ISA04 - Password, 10 spaces if no password                                  
        InterchangeSenderIdQualifier =
          "01", //ISA05 01=Duns 14=Duns plus suffix 20=Health insurance number (HIN) 27=CMS carrier ID number 28=CMS fiscal intermediary ID number 29=CMS Medicare provider /supplier ID 30 U.S. federal tax ID 33=NAIC ID ZZ=Mutually defined. 
        ...
        InterchangeReceiverId =
          "9088877320000  ", //ISA08 - Depends on who is processing it. Variable by company. We need to get a table of these values 
      ...
      Debug.WriteLine(
        "ISA*00*          *00*          *01*9012345720000  *01*9088877320000  *020816*1144*U*00401*000000031*0*T*:~IEA*0*000000031~");

      Debug.WriteLine(Message.SerializeToX12(false));
      Assert.AreEqual(
        "ISA*00*          *00*          *01*9012345720000  *01*9088877320000  *020816*1144*U*00401*000000031*0*T*:~IEA*0*000000031~",
        Message.SerializeToX12(false));
      ...
      Assert.AreEqual(
        "GS*HS*901234572000*908887732000*20070816*1615*31*X*00501X092A1~GE*0*31~",
        Group.SerializeToX12(false));
      ...
      BHTSegment.BHT01_HierarchicalStructureCode =
        "0022"; //BHT01 - Order for structure: 22 - Information Source, Information Reciever, Subscriber, Dependent 

      BHTSegment.BHT02_TransactionSetPurposeCode =
        "13"; //BHT02 - Transaction Set purpose 01- Cancel (Cancel a previous 270), 13 - Request

      BHTSegment.BHT03_ReferenceIdentification =
        "10001234"; //BHT03 - Value used to track this request (From our side) Max 50 char

      BHTSegment.BHT04_Date =
        DateTime.Parse(
          "2007/08/16"); //BHT04 - CCYYMMDD (CC = First two digits of year 20..) - When transaction was created
      ...
      Assert.AreEqual(
        "ST*270*1234~BHT*0022*13*10001234*20070816*1319*00~SE*3*1234~",
        TransactionSet.SerializeToX12(false));
      ...
      var HL1InformationSourceLevel =
        TransactionSet.AddHLoop(
          "1",
          "20",
          true); //"1" is HL01 the Id number for this loop,  "20" is HL02 - "20" means it is an info source
      ...
      Assert.AreEqual(
        "HL*1**20*1~NM1*PR*2*ABC BILLING SERVICE*****PI*842610001~",
        HL1InformationSourceLevel.SerializeToX12(false));
      ...
      var HL2SourceLoop =
        HL1InformationSourceLevel.AddHLoop("2", "21", true); //This is the 2100 Loop - The receiver information
      ...
      Assert.AreEqual(
        ...
        HL2SourceLoop.SerializeToX12(false));
      ...
      var Birthday = Member.AddSegment(new TypedSegmentDMG());
      Birthday.DMG02_DateOfBirth = DateTime.Parse("05/19/1943");

      var SubscribeDate = Member.AddSegment(new TypedSegmentDTP());
    ...
    public void Transform270ToHtml()
    {
      ...
      var html = htmlService.Transform(new StreamReader(ediFile).ReadToEnd());
