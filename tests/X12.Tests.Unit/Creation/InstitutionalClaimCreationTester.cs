...
IEA*1*000000031~";
    ...
    {
      var interchange = new Interchange(date, 31, false);
    ...
    {
      var interchange = CreateSample1InterChange(DateTime.Parse("2002-08-16 11:44AM"));
      var fg = interchange.AddFunctionGroup("HC", DateTime.Parse("2007-08-16 4:15PM"), 31);
    ...
    public void SerializeSegmentSet()
    {
      ...
      Trace.Write(xml);
      var copy = SegmentSet.Deserialize(xml);
      var isaCopy = copy.Segments.FirstOrDefault(s => s.SegmentId == "ISA");
    ...
    {
      var date = DateTime.Parse("2002-08-16 11:44AM");
      var interchange = CreateSample1InterChange(date);

      var actualX12 = interchange.SerializeToX12(true);
    ...
    {
      var interchange = CreateSample1WithFunctionGroup();
    ...
    {
      var interchange = CreateSample1WithFunctionGroup();
      var transaction = interchange.FunctionGroups.First().AddTransaction("837", "0034");
      var bht = transaction.AddSegment("BHT*0019*00*3920394930203*20070816*1615*CH");
    ...
    public void TransactionCreationWithSegmentToStringTest()
    {
      var interchange = CreateSample1WithFunctionGroup();
      var transaction = interchange.FunctionGroups.First().AddTransaction("837", "0034");
      var bht = transaction.AddSegment("BHT");
    ...
    public void ClaimCreationTest()
    {
      var interchange = CreateSample1WithFunctionGroup();
      var transaction = interchange.FunctionGroups.First().AddTransaction("837", "0034");
      var bhtSegment = transaction.AddSegment("BHT*0019*00*3920394930203*20070816*1615*CH");
      var refSegment = transaction.AddSegment("REF*87*004010X096A1");
      var senderLoop = transaction.AddLoop("NM1*41*2*HOWDEE HOSPITAL*****XX*0123456789");
      senderLoop.AddSegment("PER*IC*BETTY RUBBLE*TE*9195551111");
      var receiverLoop = transaction.AddLoop("NM1*40*2*BLUE CROSS BLUE SHIELD OF NC*****46*987654321");
      var providerLoop = transaction.AddHLoop("1", "20", true);
      ...
      Assert.AreEqual(
        new StreamReader(Extensions.GetEdi("INS._837I._4010.Example1.txt")).ReadToEnd(),
    ...
    public void ElementValidationTwoArgsTester()
    {
      try
      {
        throw new ElementValidationException(
          "Element {0} cannot contain the value '{1}' with the segment terminator.",
          "NM1",
          "AB~CD");
      }
      catch (ElementValidationException exc)
    ...
    public void ElementValidationFiveArgsTester()
    {
      ...
      {
        throw new ElementValidationException(
          "Element {0} cannot contain the value '{1}' with the segment terminator {2}. Use a value without delimiters {2} {3} or {4}.",
          "NM1",
          "AB~CD",
          '~',
          '*',
