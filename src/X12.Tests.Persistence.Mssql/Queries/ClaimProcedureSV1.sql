...
select LineLoopId = line.LoopId, Ref, Element = left(Element, 10) 
	...
	inner join SV1 profLine ON profLine.ParentLoopId = line.LoopId
	...
	cross apply(
	  ...
	  from dbo.SplitSegment(i.ComponentSeparator, profLine.[01])) sp
...
select LineLoopId = line.LoopId, Ref, Element
	...
	cross apply(
	  select line.LoopId, Ref, Element, Precedence = row_number() over (partition by line.LoopId, Ref order by line.LoopId desc) 
	  from dbo.SplitSegment(i.ComponentSeparator, profLine.[07])) sp
	where ch.ClaimLoopId > @currentClaimLoopId
		and line.LoopId = sp.LoopId and sp.Precedence = 1 
		and isnull(Element, '') != ''
...
select
	...
	Units = cast(profLine.[04] as numeric(8,2)),
	Charges = cast(profLine.[02]  as numeric(19,4)),
