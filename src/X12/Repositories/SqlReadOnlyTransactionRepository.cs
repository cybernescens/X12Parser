...
namespace X12.Repositories
{
  ...
  public class SqlReadOnlyTransactionRepository<T>
    where T : struct
  {
    ...
    protected T ConvertT(object val)
    {
      ...
      if ((typeof(T) == typeof(long?) || typeof(T) == typeof(int?)) && val == null)
        return (T) (object) 0;
    ...
    /// <returns></returns>
    public List<RepoSegment<T>> GetTransactionSetSegments(
      T transactionSetId,
      int revisionId,
...
",
    ...
    /// <summary>
    ///   This will affectively unbundle the transaction from the rest of the transaction set and show you segments related to
    ///   that loopId.
    /// </summary>
...
order by PositionInInterchange",
    ...
    {
      var set = new RepoTransactionSet<T>(
        Convert.ToChar(reader["SegmentTerminator"]),
        Convert.ToChar(reader["ElementSeparator"]),
        Convert.ToChar(reader["ComponentSeparator"]));
...
",
      ...
      using (var conn = new SqlConnection(_dsn))
      {
        ...
        cmd.Parameters.AddWithValue(
          ...
          (object) criteria.InterchangeControlNumber ?? DBNull.Value);
        ...
        var s = new List<RepoTransactionSet<T>>();
        while (reader.Read())
