...
CREATE TABLE [{0}].[Container](
    ...
    CONSTRAINT [PK_Container_{0}] PRIMARY KEY CLUSTERED ( [Id] ASC )
          ...
          Schema,
          _identitySqlType,
          _identitySqlType == SqlDbType.UniqueIdentifier ? "DEFAULT (newid())" : "IDENTITY(1,1)"));
    }
...
",
    ...
    public int ElementCountInX12CodeListTable(string elementId)
    {
      var cmd = new SqlCommand(
        string.Format(@"select count(*) from [{0}].X12CodeList where ElementId = @elementId", Schema));
...
where RevisionId = (select max([RevisionId])
    ...
    {
      ExecuteCmd(
        string.Format("ALTER TABLE [{0}].[{1}] ADD [ErrorId] [{2}] NULL;", Schema, segmentId, _identitySqlType));
    }
...
, transactionChildLoops as (  
  ...
  where RowNum = 1 and Deleted = 0
    ...
    internal void ExecuteCmd(string sql) { ExecuteCmd(new SqlCommand(sql)); }
