...
{
  public interface IParsingErrorRepo<T>
    where T : struct
  {
  ...
  public class SqlTransactionRepository<T> : SqlReadOnlyTransactionRepository<T>, IParsingErrorRepo<T>
    where T : struct
  {
    ...
    public SqlTransactionRepository(string dsn)
      : this(dsn, new SpecificationFinder(), new[] { "REF", "NM1", "N1", "N3", "N4", "DMG", "PER" }) { }

    public SqlTransactionRepository(string dsn, string schema)
      : this(dsn, new SpecificationFinder(), new[] { "REF", "NM1", "N1", "N3", "N4", "DMG", "PER" }, schema) { }
...
",
    ...
    /// <summary>
    ///   Saves the entire interchange into the database as individual segments and the relationships between the segments and
    ///   loops
    /// </summary>
    ...
    public T Save(Interchange interchange, string filename, string userName)
    {
      ...
      try
      {
        ...
        foreach (var fg in interchange.FunctionGroups)
        {
          ...
          {
            var transactionSetCode = tran.IdentifierCode;
...
select scope_identity()",
      ...
      T? parentId)
    {
      ...
      throw new InvalidOperationException(
        string.Format(
          "Loop could not be created for interchange {0} position {1}.",
          interchangeId,
          positionInInterchange));
    }
    ...
    {
      var cmd = new SqlCommand(
        string.Format("update [{0}].Interchange set HasError = 1 where Id = @interchangeId", this._schema));
...
",
    ...
    {
      var controlNumber = transaction.ControlNumber;
...
",
    ...
    private T SaveHierarchicalLoop(
      HierarchicalLoop loop,
      T interchangeId,
      T transactionSetId,
      string transactionSetCode,
...
VALUES ('{1}', {2}, '{3}', '{4}', '{5}', '{6}', '{7}', {8}) ",
      ...
      T? parentLoopId)
    {
      ...
      var cmd = new SqlCommand(GetContainerIdSql(loop.SegmentId));
      ...
      try
      {
        ExecuteCmd(
          new SqlCommand(
            GetSaveLoopSql(id.Value, loop, interchangeId, transactionSetId, transactionSetCode, parentLoopId)));
      }
      catch (Exception exc)
      {
        Trace.TraceError(exc.Message);
...
order by RevisionId desc",
        ...
        if (reader.Read())
        {
          ...
          if (previousRevisionId.HasValue &&
            Convert.ToInt64(reader["RevisionId"]) != Convert.ToInt64(previousRevisionId))
            throw new InvalidOperationException(
              string.Format(
    ...
    protected virtual void SaveSegment(
      ...
      int positionInInterchange,
      T interchangeId,
      T? functionalGroupId = null,
      T? transactionSetId = null,
      T? parentLoopId = null,
      T? loopId = null,
      int? revisionId = null,
      ...
      bool deleted = false)
    {
      if (!revisionId.HasValue ||
        SegmentHasChanged(segment, positionInInterchange, interchangeId, previousRevisionId) ||
        deleted)
      {
          ...
          using (var conn = tran == null ? new SqlConnection(this._dsn) : tran.Connection)
          {
            ...
            using (var sbc = new SqlBulkCopy(conn))
            {
              ...
              foreach (var pair in _segmentBatch._parsedTables)
              {
                ...
                sbc.DestinationTableName = string.Format("[{0}].[{1}]", this._schema, pair.Key);
    ...
    protected void ExecuteCmd(SqlCommand cmd) { _transactionDb.ExecuteCmd(cmd); }

    protected object ExecuteScalar(SqlCommand cmd) => _transactionDb.ExecuteScalar(cmd);
  }
}