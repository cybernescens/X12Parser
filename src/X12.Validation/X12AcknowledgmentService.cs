...
namespace X12.Validation
{
  public class X12AcknowledgmentService
  {
    ...
    public List<FunctionalGroupResponse> AcknowledgeTransactions(Stream x12Stream) =>
      AcknowledgeTransactions(x12Stream, Encoding.UTF8);
    ...
    public virtual List<FunctionalGroupResponse> AcknowledgeTransactions(Stream x12Stream, Encoding encoding)
    {
      ...
      using (var reader = new X12StreamReader(x12Stream, encoding, _ignoredChars))
      {
        ...
        {
          var isaElements = reader.SplitSegment(trans.IsaSegment);
          var gsElements = reader.SplitSegment(trans.GsSegment);
          var functionalIdentifierCode = gsElements[1];
          var groupControlNumber = gsElements[6];
          var versionIdentifierCode = gsElements[8];
          ...
          var response = AcknowledgeTransaction(
            reader,
    ...
    protected virtual TransactionSetResponse AcknowledgeTransaction(
      X12StreamReader reader,
      string functionalCode,
      string versionIdentifierCode,
      string transaction)
    {
      var stElements = reader.SplitSegment(transaction);
      ...
      var segments = transaction.Split(
        new[] { reader.Delimiters.SegmentTerminator },
        StringSplitOptions.RemoveEmptyEntries);

      for (var i = 0; i < segments.Length; i++)
      {
        var elements = segments[i].Split(reader.Delimiters.ElementSeparator);
        ...
        switch (segmentInfo.SegmentId)
        {
          ...
          case "SE":
            ...
            break;
          case "HL":
            var hlNumber = segmentInfo.Elements[1];
            var hlParentNumber = segmentInfo.Elements[2];
            var hlLevelCode = segmentInfo.Elements[3];
            var hlSpec =
              transactionSpec.HierarchicalLoopSpecifications.FirstOrDefault(hls => hls.LevelCode == hlLevelCode);
            ...
            {
              response.SegmentErrors.Add(
                CreateDataElementError(segmentInfo, 3, "I6", hlLevelCode)); //Code Value Not Used in Implementation
          ...
          default:
            ...
            do
            {
              var matchingLoopSpecs = currentContainer.Spec.LoopSpecifications
                .Where(ls => ls.StartingSegment.SegmentId == segmentInfo.SegmentId).ToList();
              ...
              if (matchingLoopSpecs.Count > 0)
              {
                ...
                {
                  var entityCode = elements[1];
                  matchingLoopSpec = matchingLoopSpecs.FirstOrDefault(
                    ls => ls.StartingSegment.EntityIdentifiers.Exists(ei => ei.Code == entityCode));
                }
                ...
                {
                  response.SegmentErrors.Add(
                    CreateSegmentError(segmentInfo, "6")); //Segment Not in Defined Transaction Set
              ...
              else
              {
                if (currentContainer.Spec is TransactionSpecification)
                {
                  ...
                  response.AcknowledgmentCode = AcknowledgmentCodeEnum.X_Rejected_ContentCouldNotBeAnalyzed;
                  return
                    response; // end validation if unrecognized segment encountered (cannot guarantee we are pointing at correct container)
                }
    ...
    protected virtual IEnumerable<SegmentError> ValidateContainerAgainstSpec(ContainerInformation container)
    {
      ...
      foreach (var segmentSpec in container.Spec.SegmentSpecifications.Where(ss => ss.Usage == UsageEnum.Required))
      {
        ...
        if (segmentSpec.Repeat > 0 &&
          container.Segments.Count(s => s.SegmentId == segmentSpec.SegmentId) > segmentSpec.Repeat)
          errors.Add(
            CreateSegmentError(
      ...
      foreach (var loopSpec in container.Spec.LoopSpecifications.Where(ls => ls.Usage == UsageEnum.Required))
      {
        ...
        if (loopSpec.LoopRepeat > 0 &&
          container.Containers.Count(c => c.Spec.LoopId == loopSpec.LoopId) > loopSpec.LoopRepeat)
          errors.Add(
            CreateSegmentError(
    ...
    protected virtual IList<SegmentError> ValidateSegmentAgainstSpec(SegmentInformation segmentInfo)
    {
      ...
      {
        for (var iSpec = 0; iSpec < segmentInfo.Spec.Elements.Count; iSpec++)
        {
          var elementSpec = segmentInfo.Spec.Elements[iSpec];

          if (iSpec < segmentInfo.Elements.Length - 1)
          {
            var element = segmentInfo.Elements[iSpec + 1];
        ...
        {
          var elementPosition = segmentInfo.Spec.Elements.Count + 1;
    ...
    protected SegmentError CreateDataElementError(
      SegmentInformation segmentInfo,
      int elementPositionInSegment,
