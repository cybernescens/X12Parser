...
namespace X12.Hipaa.Claims.Services
{
  public class DentalClaimToJ400FormTransformation : IClaimToClaimFormTransfomation
  {
    ...
    public DentalClaimToJ400FormTransformation(string formImagePath) { _formImagePath = formImagePath; }

    public List<FormPage> TransformClaimToClaimFormFoXml(Claim claim)
    {
      var j400 = TransformClaimToJ400(claim);
    ...
    public J400Claim TransformClaimToJ400(Claim claim)
    {
      ...
      foreach (var line in claim.ServiceLines)
      {
        var j400Line = new J400ServiceLine {
          ...
          Field28_ToothSurface = string.Join(
            "",
            line.ToothInformations.Select(t => string.Join("", t.ToothSurfaces.Select(ts => ts.Code)))),
    ...
    public List<FormPage> TransformJ400ToFormPages(J400Claim j400)
    {
      ...
      var pageCount = 1 + (j400.ServiceLines.Count - 1) / 10;
      FormPage page = null;
      for (var i = 0; i < j400.ServiceLines.Count; i++)
      {
        if (i % 10 == 0)
        {
          page = new FormPage();
        ...
        var amount = string.Format("{0:0.00}", line.Field31_Fee).Replace(".", "");
