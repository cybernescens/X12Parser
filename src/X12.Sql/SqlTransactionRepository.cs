...
namespace X12.Sql
{
  ...
  public class SqlTransactionRepository : SqlReadOnlyTransactionRepository, IParsingErrorRepo
  {
    ...
    public SqlTransactionRepository(string dsn, Type identityType)
      : this(dsn, new SpecificationFinder(), new[] { "REF", "NM1", "N1", "N3", "N4", "DMG", "PER" }, identityType) { }

    public SqlTransactionRepository(string dsn, string schema, Type identityType)
      : this(
        dsn,
        new SpecificationFinder(),
        new[] { "REF", "NM1", "N1", "N3", "N4", "DMG", "PER" },
        identityType,
        schema) { }
    ...
    public object PersistParsingError(
      ...
      string errorMessage)
    {
      var errorId = _idProvider.NextId(this._schema, "ParsingError");
...
select scope_identity()",
    ...
    private object SaveInterchange(Interchange interchange, string filename, string userName)
    {
      ...
      var interchangeId = _idProvider.NextId(this._schema, "Interchange");
...
",
    ...
    private object SaveFunctionalGroup(FunctionGroup functionGroup, object interchangeId)
    {
      ...
      var functionalGroupId = _idProvider.NextId(this._schema, "FunctionalGroup");
      var containerId = _idProvider.NextId(_commonDb.Schema, "Container");
...
",
    ...
    private object SaveTransactionSet(Transaction transaction, object interchangeId, object functionalGroupId)
    {
      ...
      var transactionSetId = _idProvider.NextId(this._schema, "TransactionSet");
      var containerId = _idProvider.NextId(_commonDb.Schema, "Container");
...
",
    ...
    private object SaveHierarchicalLoop(
      ...
      object parentLoopId)
    {
      var hlId = _idProvider.NextId(this._schema, "Loop");
      var containerId = _idProvider.NextId(_commonDb.Schema, "Container");
...
",
      ...
      cmd.Parameters.AddWithValue(
        "@parentLoopId",
        parentLoopId != null && parentLoopId != this._defaultIdentityTypeValue ? parentLoopId : DBNull.Value);
...
VALUES ('{1}', {2}, '{3}', '{4}', '{5}', '{6}', '{7}', {8}) ",
        this._schema,
        id,
        parentLoopId == this._defaultIdentityTypeValue ? "NULL" : string.Format("'{0}'", parentLoopId),
        interchangeId,
        transactionSetId,
        transactionSetCode,
    ...
    protected virtual object SaveLoop(
      ...
      object parentLoopId)
    {
      var id = _idProvider.NextId(this._schema, "Loop");
    ...
    internal virtual void ExecuteBatch(SqlTransaction tran)
    {
      ...
      if (_segmentBatch.SegmentCount > 0)
        try
        {
          ...
          _segmentBatch = new SegmentBatch(this, this._identityType);
        }
        catch (Exception exc)
        {
          Trace.WriteLine(exc.Message);
