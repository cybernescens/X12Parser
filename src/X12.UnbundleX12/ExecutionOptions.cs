    ...
    public void LoadOptions(string[] args)
    {
      if (args.Length < 3)
        throw new ArgumentException(
          @"UnbundleX12 expected 3 to 5 arguments: input filename, loop ID (or ST for transaction), output directory and an optional format string argument and include whitespace flag.  
Example1: UnbundleX12 c:\\MyEdiFile.txt 2300 c:\\Output {0}\\{1}_{2:000}{3} false
Example2: UnbundleX12 c:\\Inbound\*.edi ST c:\\Output");

      if (args.Length > 0)
      {
        var endDirectoryIndex = args[0].LastIndexOf('\\');
        var endDirectoryIndex2 = args[0].LastIndexOf('/');
